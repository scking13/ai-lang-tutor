...
async def manage_gemini_live_connection(audio_input_queue: asyncio.Queue, output_callback_func):
    ...
    """
    print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Starting.")
    global GEMINI_API_KEY_CONFIGURED, API_KEY_WARNING_PRINTED
    if not GEMINI_API_KEY_CONFIGURED:
        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Gemini API key NOT configured. Aborting.")
        if not API_KEY_WARNING_PRINTED:
            # This print is more for server console, the callback handles client notification.
            print(f"{datetime.now()} [LLM] Warning: GEMINI_API_KEY not configured. Cannot start Gemini Live session.")
            API_KEY_WARNING_PRINTED = True
        await output_callback_func({"error": "Gemini API not configured"})
        return
    print(f"{datetime.now()} [LLM] manage_gemini_live_connection: API key configured.")
    ...
    try:
        ...
        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Defining session_config.")
        ...
        # Log a key part of the config, avoiding excessive verbosity.
        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Session config model: {session_config.model if session_config else 'N/A'}")

        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Attempting to connect to Gemini Live API...")
        ...
        async with client.aio.live.connect(config=session_config) as session:
            print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Successfully connected to Gemini Live API. Session: {session}")
            # Notify the caller (WebSocket handler) that the connection is established.
            # Changed to send a more specific status message for client-side handling.
            await output_callback_func({'status': 'Gemini session connected', "session_id": session.session_id if session else "N/A"})
            ...
            async def send_audio_task():
                ...
                """
                print(f"{datetime.now()} [LLM-SendTask] Audio sending task started.")
                while True:
                    try:
                        chunk = await audio_input_queue.get()
                        if chunk is None:  # `None` is the sentinel for stopping.
                            print(f"{datetime.now()} [LLM-SendTask] Received None sentinel. Ending audio stream.")
                            break
                        if chunk: # Ensure there's data to send.
                            chunk_size = len(chunk)
                            print(f"{datetime.now()} [LLM-SendTask] Received audio chunk (size: {chunk_size} bytes). Sending to Gemini.")
                            print(f"{datetime.now()} [LLM-SendTask] Calling session.send_audio().")
                            await session.send_audio(chunk)
                    except asyncio.CancelledError:
                        print(f"{datetime.now()} [LLM-SendTask] Audio sending task cancelled.")
                        break
                    except Exception as e:
                        print(f"{datetime.now()} [LLM-SendTask] Error in send_audio_task: {e}")
                ...
                print(f"{datetime.now()} [LLM-SendTask] Audio sending task finished.")
            ...
            async def receive_messages_task():
                ...
                """
                print(f"{datetime.now()} [LLM-ReceiveTask] Message receiving task started.")
                try:
                    async for message in session.receive():
                        print(f"{datetime.now()} [LLM-ReceiveTask] Received message from Gemini session: {type(message)}")
                        print(f"{datetime.now()} [LLM-ReceiveTask] Forwarding message to output_callback_func.")
                        await output_callback_func(message)
                except asyncio.CancelledError:
                    print(f"{datetime.now()} [LLM-ReceiveTask] Message receiving task cancelled.")
                except Exception as e:
                    print(f"{datetime.now()} [LLM-ReceiveTask] Error in receive_messages_task: {e}")
                    await output_callback_func({"error": f"Error receiving message: {e}"})
                finally:
                    print(f"{datetime.now()} [LLM-ReceiveTask] Message receiving task finished.")
            ...
            print(f"{datetime.now()} [LLM] Both send and receive tasks have completed.")

    # --- Error Handling for Connection and Session ---
    except genai_types.RpcError as rpc_error: # Specific gRPC errors from the API
        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Gemini RPC Error: {rpc_error}")
        ...
        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: General Exception: {e}")
        ...
        print(f"{datetime.now()} [LLM] manage_gemini_live_connection: Exiting.")
